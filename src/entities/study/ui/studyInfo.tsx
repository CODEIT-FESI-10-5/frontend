...
export default function StudyInfo({
  ...
  return (
    <div className="flex w-full">
      <div className="flex flex-1 flex-col">
        ...
        <div className="mb-12 flex items-end justify-between">
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="flex -space-x-12">
              {members.slice(0, 4).map((member, index) => (
                <div
                  key={member.id}
                  className="border-text-secondary relative h-26 w-26 overflow-hidden rounded-full border-2 sm:h-32 sm:w-32"
              ...
              {/* 추가 멤버가 있을 경우 +숫자 표시 */}
              {members.length > 4 && (
                <div className="border-text-secondary z-30 flex h-26 w-26 items-center justify-center rounded-full border-2 bg-gray-600 sm:h-32 sm:w-32">
                  <span className="m-label-medium sm:label-large text-white">
                    +{members.length - 4}
                  </span>
                </div>
              )}
            ...
            <span
              ref={memberTextRef}
              className="text-text-primary m-label-small sm:label-small cursor-pointer underline"
          ...
          <div className="text-text-primary flex items-center justify-center gap-6 px-10">
            <span className="m-title-medium sm:headline-medium">
              {teamProgress}%
            </span>
            <span className="m-label-small sm:label-small">달성중</span>
          </div>
        </div>
        {/* Progress 바 */}
        {/* Progress Bar Container */}
        <div className="h-10 w-full overflow-hidden rounded-full bg-[#e1e1e1] backdrop-blur-sm sm:h-14">
      ...
      <div className={cn('', 'hidden', 'w-220 sm:block')}></div>
      ...
      <div
        className={cn(
          ...
          'sm:right-20 sm:-bottom-26 sm:px-18 sm:py-14',
        )}
        onClick={handleCopyInvite}
        title="클릭 시 복사"
      >
        <span className={cn('', 'm-body-large', 'sm:title-medium')}>
          초대 코드 {inviteLink}
        </span>
        <CopyIcon width={24} height={24} className={cn('hidden', 'sm:block')} />
      </div>
    </div>
  );
}
