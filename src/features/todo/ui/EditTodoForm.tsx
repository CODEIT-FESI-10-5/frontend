...
import { useEditTodoStore } from '@/features/todo/model/store/editTodoStore';

const TODOLIST_ID = '12345';
const IS_ADMIN = true;

export default function EditTodoForm() {
  const { content, setContent } = useEditTodoStore();
  const { isInvalid, setIsInvalid } = useEditTodoStore();
  const { isShared, toggleIsShared } = useEditTodoStore();
  const { resetField, toggleEditMode } = useEditTodoStore();
