...
export default function TodoList({ dashboard }: { dashboard: Dashboard }) {
  ...
  const progress = calculateProgress(dashboard.studyGoal.mytodoList);
    ...
    timeoutRef.current = setTimeout(async () => {
      if (newTitle !== dashboard.studyGoal.title && newTitle.trim() !== "") {
        try {
          ...
          // 실패 시 원래 제목으로 되돌리기
          setTitle(dashboard.studyGoal.title);
        } finally {
          setIsUpdating(false);
        }
      }
  ...
  if (dashboard.studyGoal.mytodoList.length === 0 && !title) {
    return (
  ...
  //Todo 리스트만 없을때
  if (dashboard.studyGoal.mytodoList.length === 0) {
    return (
      <div className="w-full h-[540px] max-w-[534px] min-w-0 grow shrink p-6 rounded-md bg-[#353535]">
        {/* 스터디 목표 제목 수정 가능하게 input으로 구현 */}
        <div className="relative">
        ...
        {/*Todo 생성 이동 */}
        {/*Todo href 링크 수정 */}
        <Link href="/dashboard/todo/create" className="mt-4 font-normal text-base flex text-[#f5f5f5] flex-col items-center justify-center gap-3 bg-[#2c2c2c] p-8 rounded-xl border-2 border-dashed border-[#454545]">
      ...
      <div className="p-6 rounded-md bg-[#353535]">
        ...
        <div className="relative mt-4">
          ...
          <div className="relative bg-[#4a4a4a] rounded-full h-7 overflow-hidden">
            ...
            {/* 왼쪽 텍스트 (날짜/시작시간) */}
            <div className="absolute left-3 top-1/2 transform -translate-y-1/2 text-white text-sm font-semibold">{dashboard.studyGoal.completedCt} 완료</div>
        ...
        <div className="flex flex-col gap-10 mt-10">
          ...
          <div className="flex flex-col gap-2">
            ...
            {dashboard.studyGoal.mytodoList
            ...
            {/* <Todo todo={dashboard.studyGoal.mytodoList[0]}></Todo> */}
          </div>
          {/*진행중인 투두 */}
          <div className="flex flex-col gap-2">
            ...
            {/* <Todo todo={dashboard.studyGoal.mytodoList[1]}></Todo> */}
          </div>
        </div>
        {/*전체 보기 이동 */}
